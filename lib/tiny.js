...
class Model {
    ...
    static checkType(config, value) {
        ...
        if (!hasNull && config.type === ParamsType.array) {
            ...
            if (value.length > config.arrayMaxLength)
                return new ModelResult(false, 'Array exceeds limit');
            const array = [];
            for (let i = 0; i < value.length; i++) {
                const item = value[i];
                const _result = Model.checkType({ type: config.arrayType, typeErrorMessage: config.arrayTypeErrorMessage }, item);
                if (_result === null || _result === void 0 ? void 0 : _result.valid) {
        ...
        if (!hasNull && config.type === ParamsType.number && isNaN(parseFloat(value))) {
            ...
            value = parseFloat(value); // 数字为字符串数字时，做处理
        }
        ...
        if (!hasNull && Model.prototype.isPrototypeOf((_a = config.type) === null || _a === void 0 ? void 0 : _a.prototype)) {
 ...
 */
class ModelResult {
    constructor(valid, message, value) {
        this.valid = false;
        this.message = '';
        this.value = {};
        this.valid = valid;
 ...
 * 设置此字段为数组类型
 */
function ArrayCheck(type, message, maxLength) {
    return function (target, propertyKey) {
        _checkParamsConfigExist(target, propertyKey);
        ...
        target.constructor[ModelConfigCache][propertyKey].arrayMaxLength = maxLength !== null && maxLength !== void 0 ? maxLength : 1000;
    };
}
/**
 * 添加类型错误提示语
